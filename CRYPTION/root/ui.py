	...
	def ShowBetween(self, start, end):
		...
		for i in xrange(len(self.lines)):
			if i < start or i > end:
				...
				self.lines[i].SetPosition(0, height - (end - start + 1) * self.LINE_HEIGHT + (i - start) * self.LINE_HEIGHT)
				self.lines[i].Show()
...
class Table(Window):
	...
	def __BuildLine(self, colList):
		...
		for i in xrange(len(colList)):
			line.AppendCol(str(colList[i]), int(self.GetWidth() * self.colSizePct[i] / 100))
		return line

	def SetHeader(self, colList):
	...
	def OnUpdate(self):
		...
		if overLine < 0 or overLine >= self.viewLineCount:
			if overLine == -1 and self.headerLine != None:
				width = 0
				headerColIndex = 0
				for i in xrange(len(self.colSizePct)):
					width = int(self.GetWidth() * self.colSizePct[i] / 100)
					if xMouse <= x + width:
						break
					headerColIndex += 1
					x += width
...
class ListBox(Window):
	...
	def OnMouseLeftButtonDoubleClick(self):
		if self.selectedLine != -1:
			self.doubleClickEvent(self.keyDict.get(self.selectedLine, 0))
...
class PythonScriptLoader(object):
	...
	def LoadChildren(self, parent, dicChildren):
		if dicChildren.has_key("style"):
			for style in dicChildren["style"]:
				parent.AddFlag(style)

		if not dicChildren.has_key("children"):
			return False
	...
	def LoadElementButton(self, window, value, parentWindow):
		...
		if value.has_key("tooltip_text"):
			if value.has_key("tooltip_x") and value.has_key("tooltip_y"):
	...
	def LoadElementExpandedImage(self, window, value, parentWindow):
		if not self.CheckKeyList(value["name"], value, self.EXPANDED_IMAGE_KEY_LIST):
			return False

		window.LoadImage(value["image"])

		if value.has_key("x_origin") and value.has_key("y_origin"):
			window.SetOrigin(float(value["x_origin"]), float(value["y_origin"]))

		if value.has_key("x_scale") and value.has_key("y_scale"):
			window.SetScale(float(value["x_scale"]), float(value["y_scale"]))

		if value.has_key("rect"):
	...
	def LoadElementSlot(self, window, value, parentWindow):
		...
		if value.has_key("image_r") and \
			value.has_key("image_g") and \
			value.has_key("image_b") and \
			value.has_key("image_a"):
	...
	def LoadElementGridTable(self, window, value, parentWindow):
		...
		xBlank = 0
		yBlank = 0
		if value.has_key("x_blank"):
			xBlank = int(value["x_blank"])
		if value.has_key("y_blank"):
