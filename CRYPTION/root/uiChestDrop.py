...
class ChestDropWindow(ui.ScriptWindow):
	...
	def SetOpenAmount(self, amount):
		...
		self.openChestButtonSingle.SetText("%d kutu acilacak" % self.openAmount)
		# self.openChestButtonMultiple.SetText("%d kutu acilacak" % self.openAmount)
	...
	def OnClickOpenChest(self):
		import dbg
		dbg.TraceError("pos %d" % self.invItemPos)
		...
		dbg.TraceError("count %d" % itemCount)
	...
	def RefreshItems(self, chestVnum):
		...
		if not self.chestDrop.has_key(self.currentChest):
			chat.AppendChat(chat.CHAT_TYPE_INFO, "<Hata> Mevcut olmayan sandik.")
			self.Close()
			return
