...
class TargetBoard(ui.ThinBoard):
	...
	def __init__(self):
		...
		dropButton.SetUpVisual("d:/ymir work/ui/public/question_mark_1.tga")
		dropButton.SetOverVisual("d:/ymir work/ui/public/question_mark_2.tga")
		dropButton.SetDownVisual("d:/ymir work/ui/public/question_mark_3.tga")
		dropButton.SetPosition(44, 13)
		...
		dropButton.SetEvent(ui.__mem_func__(self.OnPressedDropButton))
		dropButton.Show()
		...
		self.buttonDict["Disband from war"].SAFE_SetEvent(self.__OnClickDisbandButton)
	...
	def OnPressedDropButton(self):
		if self.dropWindow.IsShow():
			self.dropWindow.Close()
			return
		self.dropWindow.Open()
	def AddDropItem(self, slot, itemVnum):
		self.dropWindow.AddItem(slot, itemVnum)

	def CreateItemsDrop(self):
		self.dropWindow.CreateItemsDrop()

	def AppendBonuses(self, value):
		self.dropWindow.AppendBonuses(value)
	...
	def UpdatePosition(self):
		...
		self.dropWindow.SetPosition(wndMgr.GetScreenWidth()/2 - self.dropWindow.GetWidth()/2, self.GetHeight() + 10)
	...
	def __OnClickDisbandButton(self):
		disabandQuestion = uiCommon.QuestionDialog()
		disabandQuestion.SetText("Do you want kick %s named player from guild war?" % (self.GetTargetName()))
		disabandQuestion.SetAcceptEvent(ui.__mem_func__(self.DisbandPlayer))
		disabandQuestion.SetCancelEvent(ui.__mem_func__(self.CancelDisband))
		disabandQuestion.Open()
		self.disabandQuestion = disabandQuestion

	def DisbandPlayer(self):
		self.disabandQuestion.Close()
		net.SendChatPacket("/disband_gwar %s" % (self.GetTargetName()))

	def CancelDisband(self):
		self.disabandQuestion.Close()

	def __IsGuildWarMap(self):
		if str(background.GetCurrentMapName()) in ["metin2_map_t%d" % i for i in range(1, 6)]:
			return True
		return False
	...
	def ShowDefaultButton(self):
		...
		if self.__IsGuildWarMap():
			if guild.IsMemberByName(self.GetTargetName()) and player.GetGuildID() == chr.GetGuildID(self.vid):
				if player.GetMainCharacterName() == guild.GetGuildMasterName():
					self.showingButtonList.append(self.buttonDict["Disband from war"])
...
class DropWindow(ui.ScriptWindow):
	def __init__(self):
		...
		self.bonuses_table = [
			[(1 << 0), "Hayvanlara karsi,"],
			[(1 << 1), "Olumsuzlere karsi,"],
			[(1 << 2), "Seytanlara karsi,"],
			[(1 << 3), "Yari insanlara karsi,"],
			[(1 << 4), "Orklara karsi,"],
			[(1 << 5), "Mistiklere karsi,"],
		]
	...
	def LoadWindow(self):
		...
		self.Board.SetSize(270, 20)
		self.Board.SetPosition(0, 0)
		self.Board.Show()
		self.Board.SetTop()

		self.slot1 = ui.GridSlotWindow()
		self.slot1.SetParent(self)
		self.slot1.SetPosition(6, 20)
		self.slot1.SetOverInItemEvent(ui.__mem_func__(self.__OnOverInItem))
		self.slot1.SetOverOutItemEvent(ui.__mem_func__(self.OverOutItem))
		self.slot1.Show()

		self.bonuses = ui.TextLine()
		self.bonuses.SetParent(self.Board)
		self.bonuses.SetPosition(135, 5)
		self.bonuses.SetFontColor(1.0, 0.5, 0.1)
		self.bonuses.SetText("Bonuslar: Mevcut degil")
		self.bonuses.SetHorizontalAlignCenter()
		self.bonuses.Show()
	...
	def CalculateSize(self):
		...
		for item in self.items_3:
			slot = self.GetEmptySlot(3, item)
		for item in self.items_2:
			slot = self.GetEmptySlot(2, item)
		for item in self.items_1:
			slot = self.GetEmptySlot(1, item)
		for items in self.items:
			size = max(size, items)
		self.slot1.ArrangeSlot(0, 8, (size/8)+1, 32, 32, 0, 0)
		self.slot1.SetSlotBaseImage("d:/ymir work/ui/public/Slot_Base.sub", 1.0, 1.0, 1.0, 1.0)
		self.slot1.SetSize(8*32, 32*((size/8)+1))
		self.SetSize(270, (32*((size/8)+1))+30)
		self.Board.SetSize(270, (32*((size/8)+1))+30)
		self.items = {}
	...
	def CreateItemsDrop(self):
		...
		for item in self.items_3:
			slot = self.GetEmptySlot(3, item)
			self.slot1.SetItemSlot(slot, item, 0)
		for item in self.items_2:
			slot = self.GetEmptySlot(2, item)
			self.slot1.SetItemSlot(slot, item, 0)
		for item in self.items_1:
			slot = self.GetEmptySlot(1, item)
			self.slot1.SetItemSlot(slot, item, 0)
	def GetEmptySlot(self, size, vnum):
		for i in xrange(80):
			if self.IsEmptyItemGrid(i, size, vnum):
				return i
		return 99
	
	def IsEmptyItemGrid(self, slot, size, vnum):
		if slot in self.items:
			return False
		else:
			for i in xrange(size):
				self.items[slot+(i*8)] = vnum
			return True
	...
	def AppendBonuses(self, value):
		if value > 0:
			name = "Bonuses: "
			for i in xrange(6):
				if self.bonuses_table[i][0] & value:
					name = name + self.bonuses_table[i][1]
			self.bonuses.SetText(name)

	def __OnOverInItem(self, slotIndex):
		self.tooltipItem.SetItemToolTip(self.items[slotIndex])
