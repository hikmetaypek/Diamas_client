...
class LoadingWindow(ui.ScriptWindow):
	...
	def Open(self):
		...
		rateo = float(wndMgr.GetScreenHeight()) / float(self.loadingImage.GetHeight())
		self.loadingImage.SetScale(rateo, rateo)
		self.loadingGage.SetPercentage(2, 100)

		if rateo != 16.0 / 9.0:
			xFix = (self.loadingImage.GetWidth() - wndMgr.GetScreenWidth()) / 2
			self.loadingImage.SetPosition(-xFix, 0)
