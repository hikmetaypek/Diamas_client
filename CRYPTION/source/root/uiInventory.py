...
import uiPickMoney
import uiCommon
import uiPrivateShopBuilder # °³ÀÞ»óÞ¡ ¿­µ¿¾È ItemMove ¹æÞö
import localeInfo
import constInfo
import ime
import wndMgr
...
class CostumeWindow(ui.ScriptWindow):
	...
	def __init__(self, wndInventory):
		...
		if not app.ENABLE_COSTUME_SYSTEM:
			exception.Abort("CostumeWindow abort. Costume system isn't enabled")
			return

		if not wndInventory:
			exception.Abort("CostumeWindow wndInventory parameter must be set to InventoryWindow")
			return
...
class BeltInventoryWindow(ui.ScriptWindow):
	...
	def __init__(self, wndInventory):
		...
		if not app.ENABLE_NEW_EQUIPMENT_SYSTEM:
			exception.Abort("BeltInventoryWindow abort. Belt inventory isn't enabled")
			return

		if not wndInventory:
			exception.Abort("BeltInventoryWindow wndInventory parameter must be set to InventoryWindow")
			return
	...
	def CloseInventory(self):
		...
		self.AdjustPositionAndSize()
	## ÇöÀç ÀÞº¥Åä¸® À§Ä¡¸¦ ±âÞØÀ¸·Þ BASE À§Ä¡¸¦ °è»ê, ¸®ÅÞ.. ¼ıÀÚ ÇÞµåÄÚµùÇÞ±â Þ¤¸» ½ÈÞö¸¸ ¹æ¹ıÀÌ ¾ø´Ù..
...
class InventoryWindow(ui.ScriptWindow):
	...
	sellingSlotNumber = -1
	isLoaded = 0
	isOpenedCostumeWindowWhenClosingInventory = 0		# ÀÞº¥Åä¸® ´İÀ» ¶§ ÄÚ½ºÃõÀÌ ¿­·ÞÀÖ¾ú´ÂÞö ¿©ºÞ-_-; ³×ÀÌ¹Ö ¤¸¤µ
	isOpenedBeltWindowWhenClosingInventory = 0		# ÀÞº¥Åä¸® ´İÀ» ¶§ º§Æ® ÀÞº¥Åä¸®°¡ ¿­·ÞÀÖ¾ú´ÂÞö ¿©ºÞ-_-; ³×ÀÌ¹Ö ¤¸¤µ
	...
	def __init__(self):
		...
		self.isOpenedBeltWindowWhenClosingInventory = 0		# ÀÞº¥Åä¸® ´İÀ» ¶§ º§Æ® ÀÞº¥Åä¸®°¡ ¿­·ÞÀÖ¾ú´ÂÞö ¿©ºÞ-_-; ³×ÀÌ¹Ö ¤¸¤µ
	...
	def Show(self):
		...
		# ÀÞº¥Åä¸®¸¦ ´İÀ» ¶§ ÄÚ½ºÃõÀÌ ¿­·ÞÀÖ¾ú´Ù¸é ÀÞº¥Åä¸®¸¦ ¿­ ¶§ ÄÚ½ºÃõµµ °°ÀÌ ¿­µµ·Þ ÇÔ.
		if self.isOpenedCostumeWindowWhenClosingInventory and self.wndCostume:
			self.wndCostume.Show()

		# ÀÞº¥Åä¸®¸¦ ´İÀ» ¶§ º§Æ® ÀÞº¥Åä¸®°¡ ¿­·ÞÀÖ¾ú´Ù¸é °°ÀÌ ¿­µµ·Þ ÇÔ.
		if self.wndBelt:
	...
	def Hide(self):
		...
		if self.wndCostume:
			self.isOpenedCostumeWindowWhenClosingInventory = self.wndCostume.IsShow()			# ÀÞº¥Åä¸® Ã¢ÀÌ ´İÈú ¶§ ÄÚ½ºÃõÀÌ ¿­·Þ ÀÖ¾ú´Â°¡?
			self.wndCostume.Close()

		if self.wndBelt:
			self.isOpenedBeltWindowWhenClosingInventory = self.wndBelt.IsOpeningInventory()		# ÀÞº¥Åä¸® Ã¢ÀÌ ´İÈú ¶§ º§Æ® ÀÞº¥Åä¸®µµ ¿­·Þ ÀÖ¾ú´Â°¡?
	...
	def OpenPickMoneyDialog(self):
		...
		if mouseModule.mouseController.isAttached():
			...
			self.dlgPickMoney.SetMax(9) # ÀÞº¥Åä¸® 990000 Þ¦ÇÑ ¹ö±× ¼öÞ¤
	...
	def RefreshBagSlotWindow(self):
		...
		for i in xrange(player.INVENTORY_PAGE_SIZE):
			...
			## ÀÚµ¿¹°¾à (HP: #72723 ~ #72726, SP: #72727 ~ #72730) Æ¯¼öÃ³¸® - ¾ÆÀÌÅÛÀÞµ¥µµ ½½·Ô¿¡ È°¼ºÈ­/ºñÈ°¼ºÈ­ Ç¥½Ã¸¦ À§ÇÑ ÀÛ¾÷ÀÓ - [hyo]
			if constInfo.IS_AUTO_POTION(itemVnum):
				# metinSocket - [0] : È°¼ºÈ­ ¿©ºÞ, [1] : »ç¿ëÇÑ ¾ç, [2] : ÃÖ´ë ¿ë·®
	...
	def SellItem(self):
		if self.sellingSlotitemIndex == player.GetItemIndex(self.sellingSlotNumber):
			if self.sellingSlotitemCount == player.GetItemCount(self.sellingSlotNumber):
				## ¿ëÈ¥¼®µµ ÆÈ¸®°Ô ÇÞ´Â ±â´É Ãß°¡ÇÞ¸é¼­ ÀÞÀÚ type Ãß°¡
	...
	def __DropSrcItemToDestItemInInventory(self, srcItemVID, srcItemSlotPos, dstItemSlotPos):
		...
		if app.ENABLE_SOULBIND_SYSTEM and item.IsSealScroll(srcItemVID):
			...
			## ÀÌµ¿½ÃÅ² °÷ÀÌ ÀåÂø ½½·ÔÀÞ °æ¿ì ¾ÆÀÌÅÛÀ» »ç¿ëÇØ¼­ ÀåÂø ½ÃÅ²´Ù - [levites]
			if player.IsEquipmentSlot(dstItemSlotPos):

				## µé°í ÀÖ´Â ¾ÆÀÌÅÛÀÌ ÀåºñÀÞ¶§¸¸
	...
	def __SendUseItemToItemPacket(self, srcSlotPos, dstSlotPos):
		# °³ÀÞ»óÞ¡ ¿­°í ÀÖ´Â µ¿¾È ¾ÆÀÌÅÛ »ç¿ë ¹æÞö
