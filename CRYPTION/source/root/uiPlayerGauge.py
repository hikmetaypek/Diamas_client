...
class PlayerGauge(ui.Gauge):
	...
	def OnUpdate(self):
		...
		newX = x - self.GetWidth() / 2
		newY = y + 5

		if not isnan(newX):
			if not isnan(newY):
				newX = int(x - self.GetWidth() / 2)
				newY = int(y + 5)

				ui.Gauge.SetPosition(self, newX, newY + isChat*17)
			else:
				dbg.TraceError("Type conversation error newY is NaN")
		else:
			dbg.TraceError("Type conversation error newX is NaN")
